...
export async function uploadMultipart(
...
): Promise<{ fileId: string; type: string }> {
  ...
  if (!session) {
    const init = await apiClientMutation<InitResponse>(
      `/api/galleries/${galleryId}/files/upload/init`,
      {
        method: "POST",
  ...
  if (missing.length > 0) {
    ...
    for (let i = 0; i < missing.length; i += 1000) {
      ...
      const res = await apiClientMutation<{
        ...
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ fileId, partNumbers: batch }),
      });
    ...
    await runPool(missing, PART_CONCURRENCY, async (n) => {
      ...
      const res = await fetch(urlMap.get(n)!, {
        method: "PUT",
        body: blob,
        signal,
      });
      ...
      if (!etag)
        throw new Error("missing ETag - check RustFS CORS ExposeHeaders");
  ...
  const parts = Object.entries(session.parts).map(([n, etag]) => ({
    partNumber: Number(n),
    etag,
  }));
  const done = await apiClientMutation<{
    ok: boolean;
    fileId: string;
    type: string;
...
export async function abortMultipart(
  ...
  if (session?.fileId) {
    await apiClientMutation(`/api/galleries/${galleryId}/files/upload/abort`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ fileId: session.fileId }),
    }).catch(() => {
      /* best-effort */
    });
  }
