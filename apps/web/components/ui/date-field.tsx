...
export function DateField({
  id, value, onChange, placeholder = 'Pick a date',
}: {
  id?: string;
  value: string;
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className={parsed ? 'text-ink-strong' : 'text-ink-subtle'}>{label}</span>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-ink-subtle"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
      </button>

      {open && (
        <div className="absolute z-30 mt-1 w-72 rounded-md border border-border bg-surface shadow-lg p-3">
          <div className="flex items-center justify-between mb-3">
            <button type="button" onClick={() => step(-1)} aria-label="Previous month" className="h-7 w-7 inline-flex items-center justify-center rounded-md text-ink-muted hover:bg-surface-2 hover:text-ink-strong">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
            </button>
            <span className="text-sm font-bold text-ink-strong">{MONTHS[view.m]} {view.y}</span>
            <button type="button" onClick={() => step(1)} aria-label="Next month" className="h-7 w-7 inline-flex items-center justify-center rounded-md text-ink-muted hover:bg-surface-2 hover:text-ink-strong">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg>
            </button>
          </div>
          <div className="grid grid-cols-7 gap-1 mb-1">
            {WEEK.map((w) => <span key={w} className="text-center text-[10px] font-bold uppercase tracking-wider text-ink-subtle py-1">{w}</span>)}
          </div>
              ...
              return (
                <button
                  ...
                  className={`h-8 w-8 inline-flex items-center justify-center rounded-md text-sm tabular-nums transition-colors ${
                    selected ? 'bg-surface-strong text-ink-inverse font-bold' : 'text-ink-strong hover:bg-surface-2'
                  }`}
                >
                  {d}
                </button>
          ...
          {parsed && (
            <div className="mt-2 pt-2 border-t border-border flex justify-end">
              <button type="button" onClick={() => { onChange(''); setOpen(false); }} className="text-xs font-bold uppercase tracking-wider text-ink-muted hover:text-negative">
                Clear
              </button>
            </div>
          )}
