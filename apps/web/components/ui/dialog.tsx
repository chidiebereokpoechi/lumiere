...
export function confirmDialog(opts: Omit<ConfirmReq, 'kind' | 'resolve'>): Promise<boolean> {
  return new Promise((resolve) => push?.({ ...opts, kind: 'confirm', resolve }) ?? resolve(false));
}
export function promptDialog(opts: Omit<PromptReq, 'kind' | 'resolve'>): Promise<string | null> {
  return new Promise((resolve) => push?.({ ...opts, kind: 'prompt', resolve }) ?? resolve(null));
}
export function alertDialog(opts: Omit<AlertReq, 'kind' | 'resolve'>): Promise<void> {
  return new Promise((resolve) => push?.({ ...opts, kind: 'alert', resolve }) ?? resolve());
}
...
export function DialogHost() {
  ...
  useEffect(() => {
    push = (r) => { if (r.kind === 'prompt') setText(r.defaultValue ?? ''); setReq(r); };
    return () => { push = null; };
  }, []);
  ...
  const onCancel = () => close(req.kind === 'prompt' ? null : req.kind === 'confirm' ? false : undefined);
  const onConfirm = () => close(req.kind === 'prompt' ? text.trim() : req.kind === 'confirm' ? true : undefined);
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4" onClick={onCancel}>
      ...
      >
        <h2 className="text-lg font-extrabold tracking-tight text-ink-strong">{req.title}</h2>
        {req.message && <p className="mt-1.5 text-sm text-ink-muted">{req.message}</p>}

        {req.kind === 'prompt' && (
          <>
            {req.label && <label className="block mt-4 mb-2 text-xs font-bold uppercase tracking-wider text-ink-muted">{req.label}</label>}
            <input
              autoFocus
              value={text}
              onChange={(e) => setText(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter' && text.trim()) onConfirm(); }}
        ...
        <div className="mt-5 flex items-center justify-end gap-3">
          {req.kind !== 'alert' && (
            <button type="button" onClick={onCancel} className="text-sm font-semibold uppercase tracking-wider text-ink-muted hover:text-ink-strong">
          ...
          <button
            ...
            className={`inline-flex items-center rounded-md border px-4 py-2.5 text-sm font-bold uppercase tracking-wider transition-colors disabled:opacity-50 ${
              danger
          ...
          >
            {req.kind === 'confirm' ? (req.confirmLabel ?? 'Confirm') : req.kind === 'prompt' ? (req.confirmLabel ?? 'Save') : 'OK'}
          </button>
        </div>
      </div>
    </div>
