  ...
  async function save() {
    ...
    try {
      ...
      const saved = draft.id
        ? await apiClientMutation<WatermarkPreset>(
        ...
        : await apiClientMutation<WatermarkPreset>("/api/watermark-presets", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body,
          });
                ...
                <div className="flex gap-2">
                  {(["text", "image"] as const).map((t) => (
                    <button
                      ...
                      type="button"
                      onClick={() => set("type", t)}
                      className={`flex-1 rounded-md border px-3 py-2 text-sm font-semibold tracking-wider transition-colors ${
                        draft.type === t
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {[
                        ...
                        <button
                          ...
                          onClick={() => set("color", c)}
                          aria-label={c}
                          className={`h-4 w-4 rounded-md border ${draft.color.toLowerCase() === c ? "ring-2 ring-accent border-accent" : "border-border"}`}
                          style={{ background: c }}
                        />
                      ))}
                    </div>
