...
- Design language follows the user's other project `~/projects/spenny/spenny.io-web/` (adopted explicitly after the earlier Apple-soft direction was set): **slate-tone surfaces** (slate-50 canvas, white cards, slate-200 borders, slate-700/900 ink), **2px borders are the primary separator**, **dense `text-xs` (12px) body** with `font-variant-numeric: tabular-nums` globally, **4px peach ring on `:focus-visible`** via box-shadow, modest rounding (`md` = 8px workhorse). Active sidebar/tab uses `surface-strong` (slate-900) bg + ring-4 accent. Buttons hover `accent` → `accent-dark` (deeper peach). Page titles `text-2xl/3xl font-extrabold`. Eyebrows `text-[0.55rem] font-extrabold tracking-[0.28em] uppercase`. **Satoshi + peach accent are NOT from Spenny** — those are explicit lumiere choices that stay regardless.
